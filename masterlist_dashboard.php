        ...
        if ($resultUsers->num_rows > 0) {
            ...
            while ($row = $resultUsers->fetch_assoc()) {
                ...
                echo "<td>
                        ...
                        <button class='delete-btn' onclick='deleteValet(" . $row['id'] . ", \"delete_user.php\")'>Delete</button>
                      </td>";
                echo "</tr>";
            }
    ...
    if ($resultValet) {
        if ($resultValet->num_rows > 0) {
            ...
            while ($row = $resultValet->fetch_assoc()) {
                ...
                echo "<td>
                        <a class='edit-btn' href='edit_valet.php?id=" . $row['id'] . "'>Edit</a>
                        <button class='delete-btn' onclick='deleteValet(" . $row['id'] . ", \"delete_valet_r.php\")'>Delete</button>
                      </td>";
                echo "</tr>";
            }
    ...
    if ($resultRegular) {
        if ($resultRegular->num_rows > 0) {
            ...
            while ($row = $resultRegular->fetch_assoc()) {
                ...
                echo "<td>
                        <a class='edit-btn' href='edit_regular.php?id=" . $row['id'] . "'>Edit</a>
                        <button class='delete-btn' onclick='deleteValet(" . $row['id'] . ", \"delete_regular_r.php\")'>Delete</button>
                      </td>";
                echo "</tr>";
            }
...
<footer>
    ...
    <script>
    function deleteValet(id) {
        var confirmDelete = confirm("Are you sure you want to delete this valet reservation?");
        if (confirmDelete) {
            // Send an AJAX request to delete the valet reservation
            jQuery.ajax({
                type: "POST",
                url: "delete_valet_r.php",
                data: { id: id },
